...
public class KinectManager : MonoBehaviour
{
	...
	// Values to track which ID (assigned by the Kinect) is rightHand 1 and rightHand 2.
...
//				break;
	...
	// recreates and reinitializes the lists of avatar controllers, after the list of avatars for rightHand 1/2 was changed
	...
	void Start()
	{
		...
		// Add each of the avatars' controllers into a list for each rightHand.
		foreach(GameObject avatar in Player1Avatars)
		{
			if(avatar != null && avatar.activeInHierarchy)
			{
...
//			KinectWrapper.UpdateKinectSensor();
			...
			// Update rightHand 1's models if he/she is calibrated and the model is active.
			...
			// Update rightHand 2's models if he/she is calibrated and the model is active.
	...
	// Assign UserId to rightHand 1 or 2.
    void CalibrateUser(uint UserId, int UserIndex, ref KinectWrapper.NuiSkeletonData skeletonData)
    {
		// If rightHand 1 hasn't been calibrated, assign that UserID to it.
		if(!Player1Calibrated)
		{
			// Check to make sure we don't accidentally assign rightHand 2 to rightHand 1.
		...
		// Otherwise, assign to rightHand 2.
