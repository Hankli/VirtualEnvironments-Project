		...
		public class DeviceDB
		{
			...
			public bool UpdateDevice(string deviceName, string name, string descr)
			{
				m_sqlView.BeginQuery ("UPDATE device SET " +
				                      "DeviceName = @name, " +
				                      "Description = @descr " +    
				                      "WHERE CAST(DeviceName AS BINARY) = @deviceName");
			...
			{
				m_sqlView.BeginQuery ("SELECT * FROM device WHERE CAST(DeviceName AS BINARY) = @deviceName");
				m_sqlView.Write("deviceName", deviceName);
				m_sqlView.CommitQuery ();
			...
			{
				m_sqlView.BeginQuery ("SELECT COUNT(1) FROM device WHERE CAST(DeviceName AS BINARY) = @deviceName");
				m_sqlView.Write ("deviceName", deviceName);
			...
			{
				m_sqlView.BeginQuery ("DELETE FROM device WHERE CAST(DeviceName AS BINARY) = @deviceName");
				m_sqlView.Write ("deviceName", deviceName);
